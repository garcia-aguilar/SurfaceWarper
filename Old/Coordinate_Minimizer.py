...
Initial code (Zwaan, Garcia-Aguilar) in:
/home/user/ownCloud/SoftMatMech/Tubulin/Code/Full 
      ...
      ###  -- Gaussian curvature  --  ###
       ...
       ###  -- Mean curvature    --  ###
        ...
        for nj, pos_j in enumerate(neighborsCoordList):        
            ...
            k = v_nb[nk]  
            pos_k = neighborsCoordList[nk]            
            someNormalVector = jnp.cross(pos_j - pos_i, pos_k - pos_i)
            dotProduct = jnp.dot(someNormalVector, S)
...
def new_energy(lstep):
    ...
    for i in range(n):
        #E0 += v[i].E
        #rm#norm = np.linalg.norm(v[i].dE)   #tmp
        #rm#if norm==0:
            #rm#norm = 1.
...
#def plot_energy(grad=False):
#    ''' Plot the total energy per run step. E_array contains energy at each time, 
...
#    #i#np.save('arrays\\array_'+grad_mode,E_array)
#    #i#np.save('arrays\\array_steps',cum_steps)    
#    plt.figure()
#    if grad:
#        plt.plot(cum_steps,dE_array,'.')
#        plt.ylabel('Total Gradient')
#    else:
#        plt.plot(cum_steps,E_array,'.')
#        plt.ylabel('Total Surface Energy')
#    
#    plt.xlabel('Relative steps')
#    
#def plot_steps():
#    ''' Plot the step used per minimization iteration. S_array contains step at each time, 
#    while M_array contains 0 at begin step and the modifier (default=1) at rest.
#    Only called in go().''' 
#    
#    global M_array
...
vc = getArray('c')#np.array([v[i].c for i in range(n)]).flatten()
