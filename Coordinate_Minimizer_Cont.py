...
Last update Mon August 15 23

v3.2
(Changes with respect to previous versions are highlighted below.
 Smaller changes are not kept track of)

v3.2-> In v3.1 (Coordinate_Minimizer_Setup.py now), simulation parameters are given as command arguments, so script cannot
be simply run from the IDE self-standingly. 
The ease now is to be able to run it through a bash script, easier to use it 
in the cluster. 
Now with a working directory diff from output (resDir)  
...
start_conf = 'h'#'c'     
bd_mode = 'u'#'f','p','a'     
grad_mode = 'fa'#'f'#'n'#'a'   #i# commented below that only 'a' greedy analyt works
lmin_method = 'fminb'   #fminb, min, mins, mins_b

step_size = 0.001 #0.0001   #Size of gradient descent update step for vertices
dx = 1e-8#11                   #For numerically determining the gradient 
noise = 0.01#0.001      #i# noise for vertex displacement in range [0,noise*l_0]
...
rh = 1.0#i#0.8 #radius
wh  = 0.5 #width
hh  = 3#5 #heigth (#i# of top line, such that total heigh is hh+wh)

width = 1 #### only preliminary before reading
widthDic = {'A':0.96, 'B':0.98, 'C':1.05, 'L0':1.06}    #L0 = l_0*8 = 0.13249511385834717*8

#Energy constants:  (IGNORE THESE HERE, they are changed further ahead)
cH = 0#1
H0 = 0#0.75*width*np.pi
cW = 0#1 
W0 = 0#0.75*width*np.pi
cL = 0#1e6           #Constant for Hooke energy E_L = cL sum((|v_i-v_j|-l_0)/2)
...
def read_saved_conf(fileName, oldFormat=False, show3d=True, addNoise=False):
  ...
  # Read file     
    ...
    n = int(lines[n_line])
    stop_at = initData+n
    read_specific_fixed = True if 'fix' in lines[initData-1][:-1].split('\t') else False 
...
#def calc_full_gradient(coordinates1DArray):
#    global v
#    
#    coordinatesArray = np.reshape(coordinates1DArray,(n,3))
#    dE_norm = 0.
...
#        neighbors = v[i].nb 
#        pos_js = [coordinatesArray[j] for j in neighbors]
#        Ei = calc_vertex_grad(i, pos_i, pos_js)
...
def g_AG(steps,df=calc_full_gradient, la_0=1e-6, convTol=CONVTOL, interrupt=False):
    ...
    recalculate'''
    global v, E_array, fresh, dE_array, Conv_array, dx, vc, runsFileName  
    cont = ''#'y'
    ...
    if fresh:
        ...
        S_array.append(0)
        Conv_array.append(0)
        with open(runsFileName, 'w') as runsFile:  
            header = '\tE\tdE\tM\tS\tConv\n'     
            it = len(E_array)-1                 
            firstLine = '{}\t{}\t{}\t{}\t{}\t{}\n'.format(it, E_array[it], dE_array[it],0,0,0)  
            runsFile.write(header)              
            runsFile.write(firstLine)                    
        fresh = 0
    ...
    with open(runsFileName, 'a') as runsFile:  
       ...
       # Check convergence
            ...
            it = len(E_array)-1    
            newRunsLine = '{}\t{}\t{}\t{}\t{}\t{}\n'.format(it, Enew, dE_array[it],1,la,displace)   
            runsFile.write(newRunsLine)           
            if displace < CONVTOL:
...
def new_energy(lstep):
    ...
    for i in range(n):
        #E0 += v[i].E
        #rm#norm = np.linalg.norm(v[i].dE)   #tmpf
        #rm#if norm==0:
            #rm#norm = 1.
        #rm#v[i].c -= v[i].dE*lstep*(1-v[i].fixed)/norm  #tmp#
        v[i].c -= v[i].dE*lstep*(1-v[i].fixed) #i# if fixed bd, don't move
        v[i].dE_prev = v[i].dE
        #rm#v[i].dE_prev = v[i].dE/norm   #tmp#
...
# Check convergence
    ...
    return 0    
        ...
        #for i in range(n):
           # vc.append(v[i].c)
        #    E += v[i].E
        #chkprint('Energy is ' + str(E))
        #chkprint('Total Energy is ' + str(total_energy()))
        
        #E_postMove = total_energy()
        #if not E_array==[]:
            #if E_postMove > E_preMove:
            #if E > E_array[-1]:
               # print('Warning: Energy increase. Continue?')
                #if cont == '':
                   # cont = input('(y/n): ')                    
               # if cont not in ['y','Y','yes','Yes']:
                   # for i in range(n):   #i# UNDO movement
                    #    v[i].c += v[i].dE_prev*step*modifier*(1-v[i].fixed)
                   # recalculate()
                   # return 0
        
       #tmp for i in range(n):        #i# ?? why this? -> in initial definition: (for restoration)
          #tmp  v[i].dE_prev = v[i].dE
...
def show(show_init_bd = False,show_bd=True, fixBox=True):
    ...
    if fixBox:        
        lxmax = np.array([v[i].c[0] for i in range(n)]).max()
        lxmin = np.array([v[i].c[0] for i in range(n)]).min()
        lymax = np.array([v[i].c[1] for i in range(n)]).max()
        lymin = np.array([v[i].c[1] for i in range(n)]).min()
        lzmax = np.array([v[i].c[2] for i in range(n)]).max()
        lzmin = np.array([v[i].c[2] for i in range(n)]).min()
        ax1.set_zlim3d(lzmin,lzmax)
        ax1.set_box_aspect((lxmax-lxmin,lymax-lymin,lzmax-lzmin))   
...
def plot_attribute(attr, fixBox=True): 
    localFig.clf()
...
def calc_localValues(i):
   ...
   # Triangle is i, j, j+1 
     ...
     ###  -- Angle at vertex j from other two angles    --  ###
        ...
        #rm#if True:    # tmp, just to check                    
        #rm#    e_ij = v[i].c - v[j].c
        #rm#    r_ij= np.linalg.norm(e_ij)
        #rm#    e_kj  =  v[k].c - v[j].c
        #rm#    r_kj = np.linalg.norm(e_kj)        
        #rm#    cos_j = np.dot(e_ij, e_kj)/r_ij/r_kj        
        #rm#    angle_at_j2 = np.arccos(cos_j)     
            

        #rm#    if abs(angle_at_j-angle_at_j2) > 1e-12:
        #rm#        print("The two j-angles at i{0} are not the same ({1}, {2})".format(i,angle_at_j,angle_at_j2))
     ...
     ###  -- Mean curvature contribution  --  ###          
        ...
        #rm#if abs(area_triangle_old-area_triangle) > 1e-16:
        #rm#    print("vertex areas differ in i{0} with j{1}".format(i,j))
        #rm#    print("Old = {0:.4} \t New = {1:.4}".format(area_triangle_old,area_triangle))
        #rm#    print("Difference = {}\n".format(area_triangle-area_triangle_old))
...
def calc_vertex_energy_grad(i):
    ...
    if not v[i].bd:         #ii# checks if boundary 
        ...
        #v[i].A * cH * v[i].dA * (v[i].H - H0)**2#v[i].dA*cH*(v[i].H-H0)**2
...
def analysis(saveData=False, offset=0, useDF=flagDataFrame):    
    ...
    global dataDF,grad_mode, n_obtuse,fresh, dataFileName 
    
    av_mean, av_gauss, av_warp, tot_area, n_obtuse, bd_area = 0,0,0,0,0,0 
    ...
    for i in range(n):
        ...
        av_warp += v[i].W*v[i].A
        tot_area += v[i].A 
        bd_area += v[i].A*v[i].bd   
        E_tot += v[i].E
        E_mean += v[i].E_H 
        E_warp += v[i].E_W
        E_hooke += v[i].E_L
    bulk_area = tot_area - bd_area if tot_area > bd_area else tot_area       
    av_mean /= bulk_area    
    av_gauss /= bulk_area   
    av_warp /= bulk_area    
    dE_norm = total_grad()
    n_obtuse = np.sum(tr_obtuse)
    Maccum = int(np.sum(np.array(M_array)))+offset   

    if fresh:
        print("\n\t***\nEvolving Helix {0}, W ={1}\ncH = cW = {2}, H0 = {3}Pi/W, W0 = {4}Pi/W, cL {5}".format(helixType,width,bendC,dim_H0,dim_W0,cS))   
        print('Resulting in the following, cH {}, H0 {}, cW {}, W0 {}, cL {}'.format(cH,H0,cW,W0,cL))
        print('((Ignore very first print of analysis()))')   
    ...
    print('\nTotal area: ' + str(round(tot_area,3)))
    ...
    if saveData:   
        if fresh: 
            with open(dataFileName, 'w') as dataFile:   
                header =  'Maccum\tE\tdE\tE_H\tE_W\tE_L\tA\tavH\tavK\tavW\tNm\tbd_A\n'                      
                dataFile.write(header)      
        else:
            with open(dataFileName, 'a') as dataFile:               
                line1 = '{}\t{}\t{}\t{}\t{}\t'.format(Maccum, E_tot,dE_norm,E_mean,E_warp)   
                line2 = '{}\t{}\t{}\t'.format(E_hooke,tot_area,av_mean)         
                line3 = '{}\t{}\t{}\t{}\n'.format(av_gauss,av_warp,n,bd_area)   
                dataFile.write(line1+line2+line3)                   
   # DF     
    if useDF:    
        dic = {'conf':start_conf}
        dic['dE'] = dE_norm
...
def plot_vsRuns(array=E_array, key='E'):
    ...
    #plt.figure()
    runsFig.clf()
    fig1 = plt.figure("runs")   #i#plt.figure()
...
def runRoutine(gSteps, nRep, tag, offset):#g_steps=[50,250,500,2000], nRepeats=[10,20,30,20]):
    global runsFileName   
    runsFileName = resDir+'runsDF'+tag+'.dat'   
    
  # Save initial fast evolution 
    for r in range(nRep):
        num = len(M_array)-1+offset
        plot_attribute('H')
        localFig.savefig(resDir+'LocalH/LocalH'+tag+'_{}.png'.format(num))
        ...
        if itisDone:
            ...
            analysis(True, offset=offset)   
            show(True)
            saveFiles(tag,resDir,offset)
            plot_attribute('H')
            localFig.savefig(resDir+'LocalH/LocalH'+tag+'_{}_CONV.png'.format(num))
            return 'it is done'
...
def runThis(routines=[[50,10],[250,20],[500,30],[1000,40]],
    ...
    tag = '_{0}_c{1}_cS{2}_H{3}_W{4}'.format(helixType,cH,cL,dim_H0,dim_W0)   
    dataFileName = resDir+'dataDF'+tag+'.dat'  
    
    if not os.path.exists(resDir+'LocalH/'):
        os.makedirs(resDir+'LocalH/')
    if not os.path.exists(resDir+'Confs/'):
        os.makedirs(resDir+'Confs/')
    
    export_conf(resDir+'conf'+tag+'_INIT{}.config'.format(offset))
    ...
    if ref:
        ...
        tag = tag+'_n{}'.format(n)
        export_conf(resDir+'conf'+tag+'_INIT.config')
        for r in range(1):
...
def endTime():
    ...
    open('./thisDONE_{}_H{}_W{}'.format(helixType,dim_H0,dim_W0), 'a').close()
atexit.register(endTime)

def init():
    global showFig, localFig, runsFig
    summary()
    print('\nNow running initial surface setup:\n')
    showFig = plt.figure("show",figsize=(9,12))#i#
    localFig = plt.figure("local",figsize=(9,9))#i#
    runsFig = plt.figure("runs")
    reset()
    ##read_saved_conf(outDir+'cp_500.config')
    analysis()   
    ##runThis(offset=0)
...
#params = [1,'B',1.0,60.0,1,0]    #To test   #CHANGE
params = sys.argv

if len(params) < 7:
    print('\n\nThere must be 5 additional arguments:\n\tHelixType\n\tc\n\tcS\n\tdimH0\n\tdimW0\tContValue\n(Helix type=A,B,C)')  
    print("\n \t\t TRY AGAIN")
    sys.exit('ERROR')
else:
    helixType = params[1]
    bendC = float(params[2])
    cS = float(params[3])
    dim_H0 = float(params[4])   
    dim_W0 = float(params[5])  
    contValue = int(params[6])

if helixType not in ['A','B','C']:
    print("The first positional argument must be either A, B or C\nCheck also that the initConf files are H_X.config")
    sys.exit('ERROR')
else:
    #width = widthDic[helixType]   
    width = widthDic['L0']   #fix to the ideal straight width without stretching


#configFile = 'H_{}.config'.format(helixType)
...
configFile = 'cp_{}.config'.format(contValue)   
if not os.path.exists(wDir+configFile):  
    print('Initial configuraton file -- {0} -- is not in\n {1}\n'.format(configFile,wDir))   
    sys.exit('ERROR')
else:
    read_saved_conf(wDir+configFile)   
#    print("\n**\nUsing file\n\t{}\nAdding noise (eta={})...\n".format(wDir+configFile,noise))
 #   add_noise(noise*10)

resDir = 'Run/Helix_{}_cS{}_H{}_W{}_from{}/'.format(helixType,cS,dim_H0,dim_W0,contValue)  
if not os.path.exists(wDir+resDir):   
    print("The path\n\t{}\ndoesn't exist yet. Creating it....".format(wDir+resDir))   
    os.makedirs(wDir+resDir)   

os.rename(wDir+configFile, wDir+resDir+'cp_{}.config'.format(contValue))
resDir = wDir+resDir   
...
#Energy constants:
cH = bendC        
H0 = dim_H0 * np.pi / width       
cW = bendC     
W0 = dim_W0 * np.pi / width       
cL = cS#1e6           #Constant for Hooke energy E_L = cL sum((|v_i-v_j|-l_0)/2)

#analysis(True)    
runThis(routines=[[4000,40],[5000,20],[6000,20],[4000,50],[10000,40]],offset=contValue)
#runThis(routines=[[1,2]],offset=contValue)
#runThis(routines=[[100,20],[1000,20],[4000,30],[8000,40]],offset=contValue)
