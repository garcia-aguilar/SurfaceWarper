...
def runRoutine(gSteps, nRep, tag, offset):#g_steps=[50,250,500,2000], nRepeats=[10,20,30,20]):
  ...
  # Save initial fast evolution 
    for r in range(nRep):
        ...
        show(True)    
        saveFiles(tag,resDir,offset)             
        if r%1 == 0:  #CHANGE more often, all, instead of every 2 or 4 
            analysis(True, offset=offset)  
        else:   
            analysis()   
    return 0
...
def init():
    ...
    reset()
    ##read_saved_conf(wDir+'cp_500.config')
    analysis()     
    ##runThis(offset=0)
...
resDir = 'Run/Helix_{}_cS{}_H{}_W{}/'.format(helixType,cS,dim_H0,dim_W0)    
...
#runThis(routines=[[10,10],[20,10],[50,20],[100,40],[200,40]],offset=0)
#runThis(routines=[[100,10],[1000,40],[3000,40],[5000,50],[8000,30]],offset=0)
runThis(routines=[[100,10],[500,20],[1000,29],[5000,40],[7000,40],[9000,30],[10000,30]],offset=0)
